...
function createTextTexture(
  ...
  canvas.width = Math.max(1, textWidth + 20);
  canvas.height = Math.max(1, textHeight + 20);
  context.font = font;
...
class App {
  ...
  createMedias(items?: GalleryItem[]) {
...
export function CircularGallery2({
  items,
  bend = 3,
  borderRadius = 0.05,
  scrollSpeed = 2,
  ...
  React.useEffect(() => {
    ...
    const font =
      fontClassName && fontClassName.length > 0
        ? `bold 38px ${fontClassName}`
        : `bold 38px Inter, ui-sans-serif, system-ui`;
  ...
  }, [items, bend, borderRadius, scrollSpeed, scrollEase, fontClassName, showText]);

  return (
    <div
      ref={containerRef}
      className={cn('relative h-[320px] sm:h-[420px] md:h-[560px] w-full overflow-hidden', className)}
      {...props}
    />
  );
}
