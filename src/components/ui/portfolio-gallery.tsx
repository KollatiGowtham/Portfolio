...
export function PortfolioGallery({ 
  ...
  return ( 
    <section 
      ...
      id="archives" 
    > 
      <div className="max-w-7xl mx-auto bg-background/50 backdrop-blur-sm rounded-[3rem] border border-border overflow-hidden shadow-2xl"> 
        {/* Header Section */} 
        <div className="relative z-10 text-center pt-10 pb-6 px-8"> 
          <h2 className="text-4xl md:text-6xl font-black font-heading text-foreground mb-6 tracking-tighter uppercase">{title}</h2> 
 
          <Link 
            to={archiveButton.href} 
            className="inline-flex items-center gap-3 bg-primary text-primary-foreground px-8 py-3 rounded-full font-black uppercase tracking-widest text-xs hover:bg-primary/90 transition-all group mb-12 shadow-xl hover:shadow-primary/40" 
          > 
        ...
        <div className="hidden md:block relative overflow-hidden h-[500px] -mb-[150px]"> 
          <div className={`flex ${spacing} pb-8 pt-40 items-end justify-center`}> 
            {images.map((image, index) => { 
              ...
              const distanceFromMiddle = Math.abs(index - middle) 
               ...
               const zIndex = isHovered ? 100 : totalImages - index 
 
               const isOtherHovered = hoveredIndex !== null && hoveredIndex !== index 
 
              // When hovering: hovered card moves to consistent top position, others move to baseline 
               const yOffset = isHovered ? -150 : isOtherHovered ? -staggerOffset + 20 : -staggerOffset 
               const rotation = isHovered ? 0 : -45
              ...
              return ( 
                <motion.div 
                  ...
                  animate={{ 
                    transform: `perspective(5000px) rotateY(${rotation}deg) translateY(${yOffset}px)`, 
                    opacity: 1, 
                  }} 
                  transition={{ 
                    duration: 0.3, // Slightly more natural transition
                    delay: index * 0.05, 
                    ease: [0.16, 1, 0.3, 1], 
                  }} 
                ...
                > 
                  <div 
                    className={cn(
                      "relative aspect-video w-64 md:w-80 lg:w-96 rounded-2xl overflow-hidden transition-all duration-500 border border-white/10",
                      isHovered ? "scale-110 shadow-[0_40px_80px_rgba(0,0,0,0.4)]" : "shadow-2xl"
                    )}
                    style={!isHovered ? { 
                      boxShadow: ` 
          ...
          <div className="grid grid-cols-1 gap-6"> 
            {images.map((image, index) => ( 
              <div 
                ...
                onClick={() => onImageClick?.(index)} 
              > 
                <div className="relative aspect-video w-full rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-lg"> 
                  <img 
                    src={image.src || "/placeholder.svg"} 
                    alt={image.alt} 
                  ...
                  {image.title && (
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-4">
                      <h3 className="text-white text-lg font-black uppercase tracking-tighter">{image.title}</h3>
                    </div>
                  )}
                </div> 
              </div> 
